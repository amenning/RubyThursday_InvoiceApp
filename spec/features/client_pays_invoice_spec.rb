...
feature 'Client pays invoice' do
  ...
  scenario 'by visiting payment page and enters cc info', js: true do
  ...
  scenario 'but sees error message if bad number', js: true do
    visit new_invoice_subscription_path(invoice)
    expect(page).to have_content invoice.name
    expect(page).to have_content 'Test Stripe Account'
    expect(page).to have_content '$1,000.00'
    ...
    expect(page).to have_content 'Sorry we encountered an error: Your card was declined.'
